...
A basic token system (per remote server) has been implemented for each operation.   
...
* The server should send one `refresh_token` in the response. (This must be implemented using one server extension).
* This token should be immediately saved by the client for the next request.
...
Response for "get_messages_for_instance":
