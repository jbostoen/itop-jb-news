...
* A background task retrieves the third party newsroom messages from a remote external server source.
  * Step 1: The first HTTP POST request sends a limited set of data (e.g. API and some identifiers) to the external server source server in order to fetching the news messages.
    * If a new third party newsroom message has been published:
      * A copy is stored on the local iTop instance.
    * If a third party newsroom message is no longer returned by the remote external server source:
  ...
  * Step 2: A second HTTP POST request is made to the external server source, containing some statistical info (read status).
...
* On the local instance, when the front-end newsroom checks for messages, it does so against its local data.
  ...
  * If the user has displayed the page with all the messages for this external server source, this will also trigger the creation of "read status" objects for this user for each message.
...
* Connections will be made from the iTop server to the external server source server.
