...
```
	'jb-news' => array(
		...
		// Interval in minutes before checking the remote external server source(s) (if the cron job is running).
		'frequency' => 60,
		...
		// OQL which should return User objects. Allows the administrator to restrict who sees the external server sources provided by this extension.
		...
		// Path to private key (only required if acting as a external server source/server).
		'private_key_file' => '/some/path/sodium_priv.key',
		
		// Path to Sodium keys (only required if acting as a external server source/server)
		'sodium' => [
        ...
        // Experimental: Specify names (one string = one name) of external server sources that should not be checked.
        'disabled_sources' => [],
